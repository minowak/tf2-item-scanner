...
class user_stats(base.json_request):
	def __init__(self, sid):
		url = ("http://api.steampowered.com/IPlayerService/GetRecentlyPlayedGames/v0001/?key=" + base.get_api_key() + "&steamid=")
		sid = str(sid)
		lsid = sid.rfind('/')
		if(lsid + 1) >= len(sid): sid = sid[:lsid]
		sid = os.path.basename(sid)
		
		super(user_stats, self).__init__(url + sid)
